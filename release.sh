...
if [[ $( gh release view --repo "${ASSETS_REPOSITORY}" "${RELEASE_VERSION}" 2>&1 ) =~ "release not found" ]]; then
  ...
  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    ...
    RELEASE_NOTES=$( gh release view --repo "${ASSETS_REPOSITORY}" "${RELEASE_VERSION}" --json "body" --jq ".body" )
    ...
    gh release edit --repo "${ASSETS_REPOSITORY}" "${RELEASE_VERSION}" --notes-file release_notes.txt
  fi
fi
